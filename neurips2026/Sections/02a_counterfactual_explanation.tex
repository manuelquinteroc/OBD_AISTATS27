...
\begin{align}
    ...
    &=\underbrace{\E_H[Y] - \E_{K} [ M_H(X) ] }_{E^{(H)}} 
     + \underbrace{\E_{K} [ M_H(X) ] - \E_K[Y]}_{U^{(H)} } \\
     \label{eq:OB_K_Counterfactual}
     &=\underbrace{\E_H[Y] - \E_{H} [ M_K(X) ] }_{U^{(K)}} 
     + \underbrace{\E_{H} [ M_K(X) ] - \E_K[Y] }_{E^{(K)}}.
\end{align} 
