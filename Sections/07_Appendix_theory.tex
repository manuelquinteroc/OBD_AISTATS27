...
\begin{lemma}
  \label{lemma:units}
  If \( \mu_H \neq \mu_K \) elementwise, we can choose units for \( X \) such that \( \E_K[X] = 0 \) and \( \E_H[X] = 1 \).
  \begin{proof}  
...
The first condition holds almost surely under the assumed Uniform$(-M,M)$ measure on the coefficients, since \( 1^\top \Delta \beta \) is a non-degenerate linear combination of continuously distributed random variables, so we can focus on the latter two.
