...
The modern explainability literature in machine learning boast a bevy of methods for attributing outcome behavior to covariates \citep{Dwivedi2023}.
...
\textcolor{red}{However, such methods are not designed to answer this data scientist's question.
They attribute variation in outcomes within in a single population to the covariates, holding the conditional distribution $Y|X$ fixed. 
But, the data scientist is interested in why an outcome varies between two populations, where the $Y|X$ may vary.}
See \cref{appendix:xAI} for more discussion on why these methods do not solve the data scientists' question.
% For example, a Shapley value analysis of $Y|X$ might suggest that that patients with severe chronic conditions have higher mortality in each of hospital $H$ and $K$. 
...
Instead, the data scientist might adapt the Oaxaca--Blinder decomposition (OBD), which attributes disparities between two groups to $X$ and $Y|X$. 
The OBD was developed for linear models \citep{oaxaca1973,blinder1973} and remains widely used in this form --- e.g., to analyze gender wage gaps \citep{paradaContzen2025}, educational attainment differences \citep{barhaim2023}, and health disparities \citep{cartwright2021}. 
Recent work has explored pairing the OBD with flexible functional decompositions \citep{quintero2025,bach2024heterogeneity,quintas2024multiply,mao2025double,flachaire2025decomposing}, but has not engaged with the index number problem; see \cref{appendix:obd_generalized} for more discussion.
...
The index number problem would be less concerning if both references always yield the same substantive conclusions;
in our example, if both indicate that differences in the distribution of risk factors widen the mortality gap between the two hospitals.
But if one reference choice finds that risk factors widens the gap while the other finds that they narrow it, the choice of reference \emph{would} matter for qualitative conclusions and subsequent decision making.
We are not aware of an investigation into whether the choice of reference groups can yield different \emph{substantive} conclusions. 
\textcolor{red}{We fill this gap by characterizing when the OBD's substantive conclusions are sensitive to the reference group, empirically documenting this sensitivity grows when the OBD is paired with more flexible regression models, and providing theory to explain this sensitivity.\footnote{We release all of our code to replicate the experiments at \url{https://anonymous.4open.science/r/ReferenceOaxacaBlinder-8579}.}}
...
\textcolor{red}{ and show that the OBD is more sensitive to reference choice when paired with flexible regression models (\cref{sec:real_example}).}
\textcolor{red}{In \cref{sec:signflips}, we demonstrate a relationship between the size of the parameter space and the sensitivity of the OBD.}
