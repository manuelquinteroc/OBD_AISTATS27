...
We show that sign reversals can occur in either the explained or unexplained component and are not solely products of model misspecification or a small sample size. 
We find that the fraction of parameter space representing sign reversals is large for linear models and grows with the dimension of the feature space.
Our empirical exercises find that sign reversals are rarer than our theoretical results suggest, as fitted linear regression parameters in real data analyses may not be well modeled by a uniform distribution over parameter space. 
Future work can determine to what extent these conclusions hold for non-uniform parameter distributions, other measures of model complexity, and nonlinear models. 
...
Our work does have one potential negative societal impact: that a malicious researcher could aim to use the choice of reference group to cherry-pick results; however we suspect (or, hope) that the majority of mis-use of the (N)OBD is from researchers unaware of its problems, and so hope that the net impact from our paper will be positive.
