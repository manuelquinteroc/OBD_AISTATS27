...
As such, the signs of \( 1^\top \beta_{H} \) and \( 1^\top \beta_{K} \) will disagree in 50\% of the parameter space.
...
Similarly, \( 1^T \Delta \beta \) is the sum of \( 2 d \) Uniform$(-M, M)$ variables, and its distribution is an Irwin-Hall($2 d$) distribution, rescaled from $[0,2d]$ to \( [-2dM, 2dM] \).

That is, let \( I_2 \sim \text{Irwin-Hall}(2) \) and \( J_{2d} \sim \text{Irwin-Hall}(2d) \) be independent.
We can represent
\begin{align*}
    &1^T\Delta \alpha = 2M (I_2 - 1) 
    &1^T\Delta \beta  = 2M (J_{2d} - d)
\end{align*}
...
\begin{align*}
    Pr& \left( \{ |1^\top \Delta \beta| > |\Delta \alpha| \}
    \cap
    \{ \sign(\Delta \alpha) \neq \sign(1^\top \Delta \beta) \} \right) \\
    =& 
    Pr \left( \{ \Delta \alpha > 0 \} \cap \{ 1^T \Delta \beta < - \Delta \alpha \} \right)
    +
    Pr \left(
    \{ \Delta \alpha < 0 \} \cap \{ 1^T\Delta \beta > - \Delta \alpha \} \right) \\
    =& 
