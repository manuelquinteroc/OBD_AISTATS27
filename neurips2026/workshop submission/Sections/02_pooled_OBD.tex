...
Existing responses to the index number problem do not resolve it without relying on arbitrary choices or compromising the interpretation of results, and there seems to be no consensus response; see \cref{sec:existing_index_number} for a detailed discussion.
...
\begin{align}	
    ...
    &=\underbrace{ \frac{1}{N_H} \sum_{n=1}^{N_H} Y_n - \frac{1}{N_K} \sum_{n=1}^{N_K} \hat{M}_H(X_n)}_{\hat{E}^{(H)}} 
     + \underbrace{\frac{1}{N_K} \sum_{n=1}^{N_K} \hat{M}_H(X_n) - \frac{1}{N_K} \sum_{n=1}^{N_K} Y_n}_{\hat{U}^{(H)} }.
\end{align}
