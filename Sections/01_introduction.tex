...
The modern explainability literature in machine learning (ML) provides a number of methods for attributing outcome behavior ***citeXai. These include Shapley values, Functional ANOVA (FANOVA), Accumulated Local Effects (ALE), and partial dependence plots (PDP). However, these methods all focus on decompositions within a single group, not on attributing differences across groups. By contrast, the Oaxaca--Blinder decomposition (OBD) exactly attributes disparities between groups to $X$ and $Y|X$. The OBD was traditionally developed only for linear models ***cite and remains widely used in this form to the present day --- e.g., to analyze gender wage gaps \citep{paradaContzen2025}, educational attainment differences \citep{barhaim2023}, and diabetes health disparities \citep{cartwright2021}. Moreover, recent work suggests the OBD might be applied in conjunction with flexible ML decompositions (Bach, Chernozhukov, Spindler; our workshop paper here too; Quintas-Martinez et al.).
