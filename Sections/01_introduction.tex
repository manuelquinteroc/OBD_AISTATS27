...
The Oaxaca--Blinder decomposition (OBD)\footnote{Sometimes referred to as the Kitagawa--Oaxaca--Blinder (KOB) decomposition \citep{kitagawa1955}} \citep{oaxaca1973,blinder1973} is widely used to analyze disparities between groups. 
...
Several responses to this asymmetry occur in the literature. Some argue that, in certain cases,
...
Some papers report results for both OBD directions \citep{oaxaca1973,blinder1973,ONeill2006, sharafRashad2016,rahimiNazari2021}. \textcolor{red}{TB: why did this originally say "at least two references" instead of two references? also why does this citation list not include some cited as reporting both below?}
Some report a (potentially weighted) average of the two results \citep{reimers1983,cotton1988,sharafRashad2016,rahimiNazari2021,Allen2022}. \textcolor{red}{TB: this originally said "some form of aggregation; we should discuss what that means precisely and whether the rephrasing here is ok}
Some report results for just one reference, with no argument for excluding the other reference \citep{zhang_2019, ayubiShahbaziKhazaei2024,paradaContzen2025,singleton2016,lamJamiesonMittinty2021, alHanawiNjagi2022}. \textcolor{red}{TB: previously said "without proper justification" for some of these. what counts as proper justification?}

A priori, it is possible that, despite the index number problem, the OBD directions always yield the same substantive conclusions;
e.g., in our example, both directions might say the SBP difference between cities is driven in part by BMI and just differ in how much.
In this case, all of the approaches above should at least yield the same general conclusions.
However, if the OBD directions could yield different substantive conclusions, the distinction in approaches \emph{would} matter;
e.g., suppose one OBD direction tells us that 
the SBP difference is driven by BMI and the other says it is not.
While the index-number problem states that the two OBD directions can give different numerical answers, we are not aware of a systematic investigation
into whether the two different directions can yield different \emph{substantive} conclusions. We fill this gap.
