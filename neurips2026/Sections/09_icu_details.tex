...
This appendix provides full details on how we constructed the U.S.\ labor force subgroups, prepared the analytic dataset, estimated the (N)OBD, identified sign reversals, and computed bootstrap standard errors for all subgroups. 
...
\begin{table}[!ht]
    ...
    \begin{tabular}{@{}llll@{}}
    \toprule
    Subset \( S \) & Group (\( H, K \)) & Outcome ($Y$) & Model Choice \\
    \midrule
    50 states + DC          & Sex (male, female)                   & Income (natural log)   & OLS                       \\
    26 industries (NAICS 2) & Race (black, white)                  & Insurance status (0/1) & Logistic (for binary $Y$) \\
                            & Nativity (native born, foreign born) &                        & lightgbm                  \\
    \bottomrule
    \end{tabular}
\end{table}

There are \( 77 \times 3 \times 2 = 462 \) combinations of these factors for continuous outcomes and \( 77 \times 3 \times 3 = 693 \) for binary outcomes, each corresponding to a particular (N)OBD that an analyst could compute.
...
For each comparison, we perform the (N)OBD by estimating $\E[Y \mid X, g, s]$ with the specified model, 
...
\begin{itemize}
    ...
    \item lightgbm (gradient-boosted trees). We set a learning rate of 0.25, number of leaves to 7, and select the number of trees by 3-fold cross-validation with early stopping at 10 rounds; we then re-fit the estimator in sample with the selected number of trees.
\end{itemize}
...
% Insurance
\begin{table}[!ht]
    \caption{Number of subgroups (out of $462$) exhibiting a sign flip in the explained and unexplained component, for the earnings outcome.}
    \label{tab:flips_census_income}
    \centering
    \small
    \begin{tabular}{lcc}
    \toprule
          & \multicolumn{1}{c}{Explained Component} & \multicolumn{1}{c}{Unxplained Component} \\
          \cmidrule(lr){2-2}\cmidrule(lr){3-3}
    Model & Total flips & Total flips \\
    \midrule
    Linear      & &  \\
    Logistic    & &  \\
    lightgbm    & &  \\
...
% 
\begin{table}[!ht]
    \caption{Number of subgroups (out of $693$) exhibiting a sign flip in the explained and unexplained component, for the health insurance outcome.}
    \label{tab:flips_census_income}
    \centering
    \small
    \begin{tabular}{lcc}
