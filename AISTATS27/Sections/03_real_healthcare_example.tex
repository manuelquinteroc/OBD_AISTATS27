...
\begin{table*}[t]
    \caption{Number of data subsets exhibiting at least one sign flip (in either the explained or unexplained component), by model\rc{, for the ICU data (out of $139$ subsets) and for the two outcomes of the U.S.\ Census data (out of $217$ and $183$ data subsets; \cref{appendix:us_labor_force})}. Columns $10\%$, $5\%$, and $1\%$ report the number of flipped subsets that are significant at the corresponding level. \rc{Dashes: logistic regression does not apply to the continuous log-income outcome.}}
...
Concerningly, more complex models \textcolor{red}{can} have even more flips; %REV tend to
 for TabPFN \textcolor{red}{on the ICU data}, we see that more than half of the \textcolor{red}{subsets} show opposite signs %REV Concerningly, we see that \emph{every} model contains at least one sign flip, and more complex models tend to have even more flips; for TabPFN, we see that more than  half of the datasets show opposite signs
 for at least one of the explained or unexplained components, and $13.7\%$ show significant sign flips at the $5\%$ level.

\textcolor{red}{One might hope that a sufficiently flexible model would make the sign flips disappear. It does not. In \cref{appendix:complexity}, we vary the complexity of XGBoost (tree depth and number of trees) and of the neural network (width and depth) while holding every other setting fixed. On the ICU data, each increase in complexity raises the number of data subsets with a sign flip, until nearly all $139$ subsets flip. On the U.S.\ Census data of \cref{appendix:us_labor_force}, the number of flips barely changes with complexity, and for one outcome deeper networks flip fewer data subsets. We therefore do not claim a single relationship between model complexity and sign flips; the pattern appears to depend on the data. But in neither dataset does increasing complexity make sign flips go away.}
