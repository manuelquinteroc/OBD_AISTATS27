...
\Delta_Y := \E_H[Y] - \E_K[Y] 
    ...
    &=\underbrace{\E_H[Y] - \E_{K} [ \E_H[Y \mid X] ] }_{E^{(H)}} 
     + \underbrace{\E_{K} [ \E_H[Y \mid X] ] - \E_K[Y]}_{U^{(H)} } \\
     \label{eq:OB_K_Counterfactual}
     &=\underbrace{\E_H[Y] - \E_{H} [ \E_K[Y \mid X] ] }_{U^{(K)}} 
     + \underbrace{\E_{H} [ \E_K[Y \mid X] ] - \E_K[Y] }_{E^{(K)}}.
\end{align} 
...
Practitioners typically estimate counterfactual means by (1) using data from $g$ to compute an estimator of $\mathbb{E}_g[Y \mid X]$ (essentially a regression), and (2) using data from $g'$ to take the sample average across the estimator from the first step.
Concretely, suppose that, for each $g \in {\{H, K\}}$, suppose we have $N_g$ observations of $\{X_n, Y_n\}$. Let $\hat{M}_g(X)$ be an estimator for $\E_g  [ Y \mid X ]$.
...
We emphasize that the use of linear regression is a modeling choice, and that it is possible to choose nonlinear estimators for $\E_g  [ Y \mid X ]$.
For instance, \citet{quintas2024multiply,flachaire2025decomposing,mao2025double} explore generalizations of the OBD that use machine learning methods to estimate $\E_g  [ Y \mid X ]$, and derive asymptotic properties for such procedures.
