...
Although many counterfactuals can be constructed, a simple and intuitive pair arises by using the other group's outcome--covariate relationship as a bridge. In particular, one could ask the following:
...
Which counterfactual is more appropriate can depend on the question of interest and carries distinct interpretations and policy implications, sometimes the counterfactual choice is driven by domain knowledge such as in labor economics where there seems to be a consensus for the gender wage gap problem \citep{}. 
However, we argue that in many cases, the choice of the reference group is not clear. For example, it is not clear that different choices of reference in the SBP-BMI example correspond to substantively different policy questions.
And we note that even in cases where the reference group seems standard, such as the gender wage-gap, some authors still make a different choice without commentary \citep{paradaContzen2025}.
...
Using these counterfactuals, the difference in mean outcomes between groups can be expressed as the distance between each observed mean and its corresponding counterfactual\footnote{Note we use the economics terms ``explained'' and ``unexplained'' in \cref{eq:OB_H_Counterfactual}, as the differences due to changes in distribution of covariates are ``explained'' away}:
%
\begin{align}
