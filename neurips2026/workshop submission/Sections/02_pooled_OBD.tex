...
Generic responses to the index number problem do not resolve it without relying on arbitrary choices or compromising the interpretation of results; see \cref{sec:existing_index_number} for detailed discussion.
\textcolor{red}{These responses include defining a new reference group by aggregating $H$ and $K$ \citep{oaxacaRansom1998,neumark1988}, and averaging the two components as a Shapley value would \citep{Shapley1953}; we show that the choice of reference group can reverse the sign of either component, so such averages combine two questionable quantities.}
