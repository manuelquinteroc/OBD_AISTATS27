...
\Delta_Y := \E_H[Y] - \E_K[Y] 
    ...
    &=\underbrace{\E_H[Y] - \E_{K} [ \E_H[Y | X] ] }_{E^{(H)}} 
     + \underbrace{\E_{K} [ \E_H[Y | X] ] - \E_K[Y]}_{U^{(H)} } \\
     \label{eq:OB_K_Counterfactual}
     &=\underbrace{\E_H[Y] - \E_{H} [ \E_K[Y | X] ] }_{U^{(K)}} 
     + \underbrace{\E_{H} [ \E_K[Y | X] ] - \E_K[Y] }_{E^{(K)}}.
\end{align} 
...
covariates. The remaining term, corresponding to changing the conditional $Y|X$ distribution,
...
First, approaches like those of \citet{oaxacaRansom1998,neumark1988} define a new reference group by aggregating $H$ and $K$; however, there are multiple ways of defining this reference group. So while this removes the choice of $H$ versus $K$ as the reference group, it introduces a new arbitrary choice.
Second, use of Shapley values \citep{Shapley1953} could call for averaging the explained and unexplained components for each reference group.
This does remove the choice of reference group.
But, as our results call into question the validity of the explained and unexplained components for either reference group, this is merely an average of two questionable quantities.
...
In particular, implementing \cref{eq:OB_H_Counterfactual,eq:OB_K_Counterfactual} requires estimating $\E_g[Y]$, typically with the sample mean, and the counterfactual mean $\E_{g'} [ \E_g  [ Y | X ] ]$, which requires more thought. 
Practitioners typically estimate counterfactual means by (1) using data from $g$ to compute an estimator of $\mathbb{E}_g[Y|X]$ (essentially a regression), and (2) using data from $g'$ to take the sample average across the estimator from the first step.
Concretely, suppose that, for each $g \in {\{H, K\}}$, suppose we have $N_g$ observations of $\{X_n, Y_n\}$. Let $\hat{M}_g(X)$ be an estimator for $\E_g  [ Y | X ]$.
Then the following equation implements \cref{eq:OB_H_Counterfactual}:
\begin{align}	
    \hat{\Delta}_Y
    \label{eq:NOBD_H_estimated}
    &=\underbrace{ \frac{1}{N_H} \sum_{n=1}^{N_H} Y_i - \frac{1}{N_K} \sum_{n=1}^{N_K} \hat{M}_H(X_n)}_{\hat{E}^{(H)}} 
     + \underbrace{\frac{1}{N_K} \sum_{n=1}^{N_K} \hat{M}_H(X_n) - \frac{1}{N_H} \sum_{i=1}^{N_H} Y_n}_{\hat{U}^{(H)} }.
\end{align}

In the literature, the term \emph{OBD} typically refers to using \cref{eq:OB_H_Counterfactual,eq:OB_K_Counterfactual} with the further choice of linear regression to compute $\hat{M}_g(X)$.
