...
To shed more light on \cref{thm:ordering_sensitivity}, we present the condition in \cref{eq:sign_flip_condition} as a ``decision tree'' of when a sign flip can occur: 
...
\begin{figure}
    ...
    \caption{Fraction of parameter space leading to sign flips of the unexplained component from \cref{prop:unexplained_fraction}. \textcolor{red}{TODO: not sure why cref is saying Theorem here, given that the actual text says Proposition. Need to fix this}}
...
%
\begin{proposition}
	Let $C_M \subset \R^{2d}$ be a cube with side length $M > 0$, and suppose $(\beta_K, \beta_H)$ jointly lie in this cube. Let $C_{flip} \subset C_M$ be the settings of $(\beta_H, \beta_K)$ that lead to a sign flip in the explained component.
...
\begin{proposition} \label{prop:unexplained_fraction}
	Let $C_M \subset \R^{2d+2}$ be a cube with side length $M > 0$, and suppose $(\beta_K, \beta_H, \alpha_K, \alpha_H)$ jointly lie in this cube.
	Let \( I_2 \sim \text{Irwin-Hall}(2) \) and \( J_{2d} \sim \text{Irwin-Hall}(2d) \) be independent random variables.
	Then after choosing units in the regression problems such that $\mu_K = 0$ and $\mu_H = 1 \in \R^d$ and centering $Y$ such that $\alpha_K = 0$, we have
...
2 Pr\left(I_2 > 1 \cap J_{2d} < \frac{2dM - I_2}{2M}\right).
 \end{equation}
 And this probability approaches $1/2$ as $d \to \infty$.
\end{proposition}
...
This discrepancy must be explained by a bias in the parameter values we see in practice.
In this section we attempt to explain this bias towards parameter values in practice that do not lead to sign flips.
