...
The Oaxaca--Blinder decomposition (OBD)\footnote{Sometimes referred to as the Kitagawa--Oaxaca--Blinder (KOB) decomposition \citealp{kitagawa1955})} \citep{oaxaca1973,blinder1973} is widely used in the econometrics literature to analyze disparities between groups as in the motivating example. 
...
Since the original paper, \citet{oaxaca1973} noted that the numerical values of the OBD components depend on the choice of reference group, known classically as the \textit{index-number problem}.
This asymmetry arises because the OBD involves computing differences in covariate means and differences in regression coefficients between the two populations; the choice of which direction to do the subtraction---implicitly setting a reference group---typically yields different results.
...
For instance, labor economists often agree on the conception that either men or a benchmark wage between women and men represents the non-discriminatory wage in the gender wage gap problem; yet some studies report women as the reference group \citep{paradaContzen2025}. 
...
We illustrate these findings with a synthetic healthcare example inspired by real data analyses and provide a complete set of interpretable conditions under which such sign flipping --- and subsequent ambiguity of the OBD --- will occur. \mqcomment{Add real case}

% First Version (Manuel)
% The Oaxaca–Blinder (OB) \citep{oaxaca1973,blinder1973} decomposition, sometimes referred to as Kitagawa–Oaxaca–Blinder (KOB; \citealp{kitagawa1955}), decomposes the difference in outcome (\(Y\)) means between two groups into a part that can be explained by differences in the covariates (\(X\)), usually known as the explained part, and a component attributable to differences in \(Y \mid X\), usually referred to as the unexplained part (also called the discrimination or structural part). The OB decomposition is a widely used method in economics and social sciences to analyze disparities between groups, such as wage gaps or educational attainment differences. It provides insights into how much of the difference can be attributed to observable characteristics versus structural differences between groups.  

% From the very beginning, authors warned that results depend on the chosen reference coefficients—the classic \textit{index-number problem} \citep{blinder1973,oaxaca1973}. Since then, two justifications dominate the literature: first, treat the two orders as ``equally valid'' and report both to bracket a range \citep{oaxaca1973,blinder1973}; second, choose a reference based on a nondiscriminatory counterfactual or depending on the question one seeks to answer \citep{fortinLemieuxFirpo2011,jann2008}. In health services research, recent papers acknowledge that even the ratio of explained to unexplained components hinges on this counterfactual choice \citep{rahimiNazari2021,meulman2025}. At the same time, classic work in sociology cautions that selecting a comparison group can be ``inherently arbitrary,'' inviting arbitrary decompositions of the residual gap \citep{jonesKelley1984}.

% To alleviate this seemingly arbitrary choice of reference group, several solutions have been proposed: symmetric averages \citep{reimers1983}, group-share weighting for the disadvantaged \citep{cotton1988}, and pooled regressions intended to approximate a competitive structure \citep{neumark1988,oaxacaRansom1994}. These approaches aim to mitigate arbitrariness in choosing a reference and provide more stable estimates of the explained and unexplained components. However, the choice of reference remains contentious, with no consensus on a single best approach. Some argue it should be grounded in theoretical or policy relevance \citep{jann2008,fortinLemieuxFirpo2011}, while others emphasize pooled or averaged structures for stability and comparability \citep{sloczynski2015,firpoWoL2017}.

% We argue that the choice of reference group is not a technical detail; it can flip the substantive conclusion. This is especially concerning in policy settings where OB guides decisions on how to address disparities. We show that \emph{sign reversals} of the unexplained (structural) component—and of the explained component—are not edge cases: they arise under realistic conditions, even with a correctly specified linear model and the same data. This challenges the robustness of conclusions drawn from OB decompositions and calls for careful consideration of reference group selection. We illustrate with a health example and a short theorem that characterizes the flip region where these shifts can occur.
