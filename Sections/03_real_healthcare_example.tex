...
In \cref{fig:hr_quartile2}, we show the fitted linear models of in-hospital mortality on admission heart rate, holding all other covariates fixed at their subgroup means. 
Indeed, we see that the signs of the slopes differ.
In Appendix \cref{tab:delta_mu_betas}, we report values for all $\beta$'s and see that there are multiple covariates for which $\beta_{men} < 0 < \beta_{women}$, and additionally, $\Delta\mu$ is positive for all of these covariates.
Put together, these changes are enough to cause a sign flip in the OBD.
