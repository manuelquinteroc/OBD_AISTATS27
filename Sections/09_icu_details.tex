...
\begin{table}[h]
    \centering
    \small
    \caption{Covariate mean differences and group-specific coefficients in HR quartile two}
    \label{tab:delta_mu_betas}
    \begin{tabular}{lrrr}
    \hline
    \textbf{Covariate} 
    & $\boldsymbol{\Delta\mu}$ 
    & $\boldsymbol{\beta_{\text{Men}}}$ 
    & $\boldsymbol{\beta_{\text{Women}}}$ \\
    \hline \\[-6pt]
    ...
    \vspace{0.5em}
    \footnotesize{
    \textit{Note:} $\Delta\mu$ denotes the difference in covariate means between men and women within HR quartile two (men minus women). 
...
\begin{table}[h]
    ...
    Subset \( S \)          & Outcome \( Y \)            & Group (\( H, K \))                     & Covariate (\(X\))        \\ \midrule
...
\begin{table}[H]
    ...
    \multirow{2}{*}{\textbf{n}} &
    \textbf{Gap} &
    \multicolumn{2}{c}{\textbf{Foreign Born Reference}} &
    \multicolumn{2}{c}{\textbf{Native Reference}} \\
    \cmidrule(lr){3-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
    ...
    \footnotesize
    \textit{Note:} Standard errors in parentheses computed via bootstrap resampling with $20{,}000$ replications.  
    P-values based on a normal approximation.  
    Significance levels: * $p<0.10$, ** $p<0.05$, *** $p<0.01$.      
\end{table}