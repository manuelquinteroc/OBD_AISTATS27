\section{When can sign reversals occur?} 
\label{sec:signflips}

Building on the real clinical case study and our simulated example, a natural next step is to ask under what conditions the explained and unexplained components flip sign, as the policy implications may hinge critically on their direction. 
\cref{eq:OB_H_decomp,eq:OB_K_decomp} make clear that the value can vary with the choice of reference group, but it is less obvious when a sign flip can occur, prompting the question: under what conditions do these components flip sign?

\paragraph{Explained component.}
We begin with the explained component. Recall from \cref{eq:OB_H_decomp,eq:OB_K_decomp} that the explained term takes the form $\Delta\mu^T\beta_g$, where $\Delta\mu := \mu_H - \mu_K$ is the common covariate mean difference and $g \in \{H,K\}$ denotes the reference group. Because $\Delta\mu$ is fixed across references, any change in sign of the explained component is entirely driven by the inner product of $\Delta\mu$ with the coefficient vectors $\beta_g$.

\begin{definition}
	Let $\Delta\mu := \mu_H - \mu_K$. The OBD \emph{sign flips the explained component} if the sign of the explained term, $\Delta\mu^T \beta_g$, depends on the reference group. That is
	\begin{equation}
        \label{eq:sign_flip_condition_explained_neq}
		\sign\!\left(\Delta\mu^T \beta_H\right) \neq \sign\!\left(\Delta\mu^T \beta_K\right).
	\end{equation}
\end{definition}

The condition and its proof under which such a sign flip occurs follows immediately from this definition.

\begin{restatable}[OBD explained sign flips]{theorem}{OBSignFlipsExplained}
    \label{thm:explained_ordering_sensitivity}
    The OBD \emph{sign flips the explained component} if and only if $\Delta\mu^T \beta_H \neq \Delta\mu^T \beta_K$, and
    \begin{equation}
    \label{eq:sign_flip_condition_explained}
        \min\{\Delta\mu^T \beta_H,\ \Delta\mu^T \beta_K\}
        < 0 <
        \max\{\Delta\mu^T \beta_H,\ \Delta\mu^T \beta_K\}.
    \end{equation}
\end{restatable}

\begin{remark}
    If either $\Delta\mu^T \beta_H = 0$ or  $\Delta\mu^T \beta_K = 0$, then one explained term equals zero.
\end{remark}

To see how this characterization applies in practice, we return to the ICU mortality and heart rate example presented in \cref{sec:real_example}. In that example, we observed a sign flip in the explained component of the OBD when using men or women as the reference group. Within HR quartile two, the covariate mean difference $\Delta\mu$ between men and women is fixed by the sample, but the coefficients governing how mortality loads on these covariate differences differ in sign across groups. In particular, the opposite directions of the heart rate coefficients for men and women shown in \cref{fig:hr_quartile2} suggest that a sign flip can occur. As a result, $\Delta\mu^T\beta_{\text{women}}$ and $\Delta\mu^T\beta_{\text{men}}$ have opposite signs, leading to a sign reversal in the explained component. A numerical breakdown of $\Delta\mu$ and the group-specific coefficients is reported in Appendix~\cref{tab:delta_mu_betas}.

What fraction of the parameter space does this represent?
The answer depends on the measure we assign to the regression coefficients.
To establish a baseline, suppose we assign a uniform distribution to \( \beta_H \) and \( \beta_K \) over some interval \( [-M, M] \).
Under the assumption that \( \mu_H \neq \mu_K \) elementwise (which holds almost surely for atomless measures), we can simplify the analysis by rescaling the covariates via \( \tilde{X} = (X - \mu_K) / (\mu_H - \mu_K) \), where the division is elementwise.%
\footnote{\textcolor{red}{Advik: we can do better than my first attempt at introducing this paragraph.}}
For the rescaled covariates, \( \Delta \mu \) is a vector of ones, so \cref{eq:sign_flip_condition_explained_neq} simplifies to \( \sign\!\left(\sum_{i} \beta_{H,i}\right) \neq \sign\!\left(\sum_{i} \beta_{K,i}\right) \).
The Uniform distributions on the regression coefficients translate to distributions on \( \sum_i \tilde{\beta}_{H,i} \) and \( \sum_i \tilde{\beta}_{K,i} \) that are symmetric around zero, so that \cref{eq:sign_flip_condition_explained_neq} corresponds to 50\% of the parameter space.

\paragraph{Unexplained component.}

Unlike the explained component, sign reversals in the unexplained component arise from the interaction between slope and intercept differences, leading to a richer set of conditions.

\begin{definition}
	Let $\Delta\beta := \beta_H - \beta_K$ and $\Delta\alpha := \alpha_H - \alpha_K$. The OBD \emph{sign flips the unexplained component} if the sign of the unexplained term, $\mu_{g'}^T \Delta\beta + \Delta\alpha$, depends on the reference group. That is
	\begin{equation}
		\sign\left( \mu_K^T \Delta\beta + \Delta\alpha \right) \neq \sign\left( \mu_H^T \Delta\beta + \Delta\alpha \right).
	\end{equation}
\end{definition}
%
We now demonstrate that sign flipping in the unexplained component of the OBD is not an isolated phenomenon.

\begin{restatable}[OBD unexplained sign flips]{theorem}{OBSignFlips}
    \label{thm:ordering_sensitivity}
    The OBD \emph{sign flips the unexplained component} if and only if $\mu_H^T \Delta\beta \neq \mu_K^T \Delta\beta$, and
    \begin{equation}
    \label{eq:sign_flip_condition}
    \min\{\mu_H^T \Delta\beta, \mu_K^T \Delta\beta\} < -\Delta\alpha < \max\{\mu_H^T \Delta\beta, \mu_K^T \Delta\beta\}.
    \end{equation}
\end{restatable}    

\begin{remark}
    If $-\Delta\alpha \in \{\mu_H^T \Delta\beta, \mu_K^T \Delta\beta\}$, then one unexplained term equals zero.
\end{remark}

In \cref{sec:sign_flip_proof}, we provide the proof of \cref{thm:ordering_sensitivity}. 

To shed more light on the utility of \cref{thm:ordering_sensitivity}, we present the condition in \cref{eq:sign_flip_condition} as a ``decision tree'' of when a sign flip can occur: 

\begin{algorithm}[H]
    \caption{Unexplained component sign-flip}
    \label{alg:decision_tree}
\begin{algorithmic}  
    \If{$\mu_H^T \Delta\beta \neq \mu_K^T \Delta\beta$}
        \If{$\sign(\mu_H^T \Delta\beta) = \sign(\mu_K^T \Delta\beta)$}
            \If{$\sign(\Delta\alpha) \neq \sign(\mu_H^T \Delta\beta)$}
                \If{$|\mu_H^T \Delta\beta| < |\Delta\alpha| < |\mu_K^T \Delta\beta|$ \textbf{or} $|\mu_K^T \Delta\beta| < |\Delta\alpha| < |\mu_H^T \Delta\beta|$}
                    \State A sign flip occurs.
                \EndIf
            \EndIf
        \ElsIf{$\sign(\mu_H^T \Delta\beta) \neq \sign(\mu_K^T \Delta\beta)$}
            \If{$\sign(\Delta\alpha) = \sign(\mu_H^T \Delta\beta)$ \textbf{and} $|\mu_K^T \Delta\beta| > |\Delta\alpha|$}
                \State A sign flip occurs.
            \ElsIf{$\sign(\Delta\alpha) \neq \sign(\mu_H^T \Delta\beta)$ \textbf{and} $|\mu_H^T \Delta\beta| > |\Delta\alpha|$}
                \State A sign flip occurs.
            \EndIf
        \EndIf
    \Else
        \State There is no sign flip.
    \EndIf
\end{algorithmic}
\end{algorithm}

To illustrate how we can make use of \cref{alg:decision_tree}, we return to the healthcare example in \cref{sec:example}.
In this example, $\mu_H$ and $\mu_K$ denote mean BMIs, both of which are positive by definition and thus $\sign{(\mu_H \Delta\beta)} = \sign{(\mu_K \Delta\beta)} = \sign{(\Delta\beta)}$. Therefore, following \Cref{alg:decision_tree}, a sign flip will occur only if $\sign{(\Delta\alpha)} \neq \sign{(\mu_H \Delta\beta)} = \sign{(\Delta\beta)}$; equivalently, it must hold that one group has a larger $\beta_g$ (more sensitive to higher BMI) but lower $\alpha_g$ (lower SBP on average).
This might occur if one group has better access to treatment but worse genetic factors, as in \cref{sec:example}.
Furthermore, a sign-flip will occur if either $\abs{\mu_H \Delta\beta} < \abs{\Delta\alpha} < \abs{\mu_K \Delta\beta}$ or $\abs{\mu_K \Delta\beta} < \abs{\Delta\alpha} < \abs{\mu_H \Delta\beta}$. This condition implies a range of values over which sign flips will occur; that is, there will be at least one choice of reference group under which improved healthcare appears to help in lowering mean SBP and another ordering under which improved healthcare appears to \emph{hurt} mean SBP.

Even though \cref{thm:ordering_sensitivity} does not restrict the magnitude of the parts of the unexplained components, in \cref{ex:unbounded} we provide an example to illustrate that these sign flips are not relegated to settings in which $\mu_g^T\Delta\beta$ are small in magnitude; in fact, we show that they can be arbitrarily large in magnitude.

Moreover, as with the explained component, sign flips corresponds to a nontrivial region of the parameter space.
We argue similarly as before, rescaling variables to simplify the analysis.
Let \( \tilde{X} = (X - \mu_K) / (\mu_H - \mu_K) \), where the division is elementwise, and let \( \tilde{Y} = Y - E_K[Y] \).
For the rescaled covariates, \( \mu_K = 0\), \( \alpha_K = 0 \), and \( \mu_H \) = 1.
The conditions in \cref{alg:decision_tree} then simplify, so that the unexplained component flips sign iff \( 1^T \Delta \beta \neq 0 \), \( \sign (\alpha_H) \neq \sign (1^T \Delta \beta) \), and \( | 1^T \Delta \beta | > |\alpha_H| \), where \( 1 \) represents a vector of ones.

To determine what fraction of the parameter space this corresponds to, we assign a uniform distribution over the interval \( [-M, M] \) to each of the regression coefficients.
Then \( 1^T \Delta \beta \neq 0 \) almost surely, and we can focus on the remaining condition.
As each entry of \( \beta_H \) and \( \beta_K \) has a uniform distribution, the quantity \( 1^T \Delta \beta \) follows an Irwin-Hall(2d) distribution rescaled to the interval \( [-2dM, 2dM] \); it is the sum of \(2d\) Uniform(-M, M) variables.

We can use this observation to compute the probability of a sign flip in the unexplained component.
Let \( I \sim \text{Irwin-Hall}(2d) \).
Then the event \( | 1^T \Delta \beta | > |\alpha_H| \cup \sign (\alpha_H) \neq \sign (1^T \Delta \beta) \) is equivalent to the union \( (\alpha_H > 0 \cap I \in [-2d, \frac{2dM - \alpha_H}{2M}]) \cup (\alpha_H < 0 \cap I \in [\frac{\alpha_H + 2dM}{2M}, 2d] ) \); that is, when the Irwin-Hall variable takes a value in its tail with the opposite sign of \( \alpha_H \).
By symmetry, this is twice the probability that \( \alpha_H \) is positive and the Irwin-Hall variable is large and negative.
That is:
\[ Pr(| 1^T \Delta \beta | > |\alpha_H|) = 2 Pr(\alpha_H > 0 \cap I < \frac{2dM - \alpha_H}{2M}). \] 
We can calculate this exactly for small \( d \), after which point the Irwin-Hall CDF becomes difficult to evaluate due to combinatorial terms.
For large \( d \), the central limit theorem allows us to approximate an Irwin-Hall(2d) distribution with a \( N(d, \frac{d}{6})\) distribution.
\cref{fig:prob_unexplained} shows how this probability grows in the dimension of the covariates, \( d \), for various choices of \( M \).

% \begin{figure}[h]
%     \centerline{\includegraphics[width=0.75\linewidth]{Sections/Figures/prob_unexplained.pdf}}
%     \caption{Probability of unexplained component sign flip under uniform measure.}
%     \label{fig:prob_unexplained}
% \end{figure}

An attentive reader may note that unexplained component sign flips are rare in practice in both of our empirical investigations, despite accounting for a large proportion of the parameter space.
While we cannot provide a definitive answer, we can offer one possible explanation.
The explanation hinges on the condition \( | 1^T \Delta \beta | > |\alpha_H| \) and the nature of the intercept term in linear regression.
First, note that the condition \( | 1^T \Delta \beta | > |\alpha_H| \) obtains most easily when many of the components of \( \Delta \beta \) are the same sign.
That is, when \( \beta_H > \beta_K \) across many coordinates; or else \( \beta_H < \beta_K \) across many coordinates.
In either cases, the differences \( \beta_H - \beta_K \) accumulate and produce a large magnitude when summed.

For sign flips to be unlikely, it should be the case that data-generating processes that produce large \( | 1^T \Delta \beta | \) also produce large \( \delta \alpha \).
One possibility is based on the fact that intercepts \( \alpha_H \) reflects the influence of variables not recorded in a researcher's data.
Suppose that there exist variables \( Z \in \mathbb{R}^{d'} \) that are distinct from \( X \).
We can show that the intercept \( \alpha_H \) depends on the relationship between \( Z \) and \( Y \).

% Show that the regression coefficient \alpha depends on the coefficients on Z
Using the relations
\begin{align*}
   \E_g[Y] &= \omega_g + \E_g[X]^\top \theta_g + \E_g[Z]^\top \gamma_g, \\    
   \E_g[Z] &= \zeta_g + \E_g[X]^\top \psi_g 
\end{align*}
we can observe that in 
\[ \E_g[Y] = \alpha_g + \E_g[X]^\top \beta_g \]
the coefficients can be written
\begin{align*}    
   &\alpha_g = \omega_g + \zeta_g^\top \gamma_g,
   &\beta_g  = \theta_g +\psi_g^\top \gamma_g
\end{align*}
and therefore 
\begin{align*}
  &\Delta \alpha = \Delta \omega + \Delta \zeta^\top \Delta \gamma,
  &\Delta \beta  = \Delta \theta + \Delta \psi^\top \Delta \gamma.
\end{align*}
Note the common component, \( \Delta \gamma \).
So when \( \Delta \beta \) is large is this because X has bona fide predictive power of its own (large \( \Delta \theta  \)) or because it's correlated with unobserved variables that truly have the predictive power (large \( \Delta \gamma \))?
If it's the second case, then both \( \Delta \alpha \) and \( \Delta \beta \) will tend to be jointly large, making it more difficult for the sign-flip condition to obtain.

This suggests a simple heuristic: measure the predictive power of the regression of \( Y \) on \( X \).
WHen this regression has a larger \( R^2 \), there is less room for unobserved covariates 
In Figures TODO we plot the distribution of \( R^2 \) in our empirical exercises and find that TODO.

%    \Cref{thm:ordering_sensitivity} shows that sign reversals in the unexplained component are not edge cases. Equation \(U^{(K)} - U^{(H)} = -B\) implies that switching the reference group shifts the unexplained term by exactly \(B\), and the explained part by \(-B\). When \(B \neq 0\), this shift can flip the diagnosis—from structural underpayment to overpayment—even with a correctly specified model. The disagreement region \(S \in (-B, 0)\) for \(B > 0\) (and its mirror for \(B < 0\)) is tight: outside this interval, both orderings agree in sign; inside it, they disagree (see \cref{imageTODO}). This window has width \(|B|\), which grows with larger covariate shifts or slope differences—both common in practice. So sign flips are not rare or pathological; they’re a general sensitivity that arises whenever \(B\) is materially different from zero. This motivates the next result.
%
%    The next corollary formalizes that the explained parts across orderings move in perfect opposition to the unexplained parts. Hence, any instability in the structural attribution is mirrored one-for-one by instability in the endowment attribution. This zero-sum coupling is central: it shows that ``credit” assigned to covariate differences depends on the arbitrary choice of reference.
%    
%
%    \begin{restatable}[]{corollary}{explainablepart}
%        \label{cor:explained_flip}
%        With \(B\) as above,
%        \(
%        E^{(K)} = E^{(H)} - B
%        \).
%        Thus the explained components flip sign iff \(E^{(H)}(E^{(H)}-B)<0\); any flip window has width \(|B|\), and
%        \(
%        (E^{(K)}-E^{(H)}) = -(U^{(K)}-U^{(H)})
%        \).
%    \end{restatable}
%
%
%It may be tempting to wonder whether this sensitivity is somehow bounded by the observed outcome gap \(\Delta_Y\). \Cref{cor:unbounded_sensitivity} in the Appendix addresses this question directly: even when \(\Delta_Y\) is held fixed, the ordering-induced swing in the structural component can be made arbitrarily large. That is, no a priori bound ties the magnitude of ordering sensitivity to the magnitude of the empirical gap itself.


\section{When do sign reversals not occur?}
\label{sec:no_sign_flips}
While \cref{thm:explained_ordering_sensitivity,thm:ordering_sensitivity} characterizes when sign reversals can arise, it is also useful to understand when they do \emph{not} occur. 
For the explained component, although differences in coefficients can in principle offset differences in covariate means, the most immediate condition under which no sign reversal occurs is when the covariate mean difference loads in the same direction under both reference models.

For the unexplained component, a sign flip intuitively requires the intercept and slope effects to pull in opposite directions across groups. When both move in the same direction, so that the group with higher baseline outcomes also has higher returns to covariates, the unexplained component preserves its sign regardless of the chosen reference group.

We formalize this idea in the following assumption. 

\begin{assumption}[Aligned intercept--slope effects]
\label{ass:no_sign_flip}
\[
    (\mu_H^T \Delta\beta)(\mu_K^T \Delta\beta) > 0 
\quad \text{and}  
\]
\[
    \sign(\Delta\alpha) = \sign(\mu_H^T \Delta\beta) = \sign(\mu_K^T \Delta\beta),
\]
\end{assumption}

\begin{proposition}
    \label{prop:no_sign_flip}
    Under \cref{ass:no_sign_flip}, the unexplained components 
    \( U^{(H)}\) and  \(U^{(K)}\) share the same sign.
    \end{proposition}

Under \Cref{ass:no_sign_flip} both unexplained components share the same sign and the OBD cannot exhibit a sign reversal. 
This assumption requires that the group with higher baseline outcomes ($\alpha_g$) also benefits more from the covariates ($\beta_g$) for \(g \in \{H,K\}\).

For example, in terms of the gender wage example introduced in \cref{sec:counterfactual_example}, \cref{ass:no_sign_flip} corresponds to settings where the group earning higher baseline wages ($\alpha_g$) also experiences higher returns to skills and education ($\beta_g$). 
Here, both intercept and slope advantages move in the same direction, so the unexplained component, presents consistent sign between reference groups. 

Similarly, for the SBP--BMI example in \cref{sec:example}, \cref{ass:no_sign_flip} would correspond to a case where the higher-resource group exhibits both a higher baseline SBP and a stronger SBP--BMI relationship than the lower-resource group. However, in the simulated data, the lower-resource group shows a steeper SBP--BMI slope while having a lower baseline, violating \cref{ass:no_sign_flip}.


