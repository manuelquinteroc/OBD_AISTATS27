...
After characterizing sign flips in \cref{sign_flip_characterizations}, we can ask how likely they are.
...
As the next two propositions demonstrate, under these choices, the fraction of parameters leading to sign flips is fairly large.
%
For the explained component, we find that, under our conditions, half of the parameter space corresponds to sign flips.
...
For the unexplained component, we find that the proportion of parameter space corresponding to sign flips nears 1/2 as the dimension of the covariate vector grows.
\begin{proposition} 
...
\begin{align} \label{eq:unexplained_fraction}
    P_d := Pr \left( \{ I_2 > 1 \} \cap \{ J_{2d} < d + 1 - I_2 \}  \right) + \nonumber \\
    Pr \left( \{ I_2 < 1 \} \cap \{ J_{2d} > d + 1 - I_2 \} \right),
\end{align}
where the probability $Pr$ is over the independent random variables $I_2, J_{2d}$.
