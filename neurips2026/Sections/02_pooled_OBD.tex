...
\label{sec:ob_real}
\textcolor{red}{ Implementing \cref{eq:OB_H_Counterfactual,eq:OB_K_Counterfactual} requires estimating observed means $\E_g[Y]$, typically with the sample mean; and counterfactual means $\E_{g'} [ \E_g  [ Y | X ] ]$, which require more thought.
Practitioners typically estimate counterfactual means by first computing an estimator of conditional expectation $\mathbb{E}_g[Y|X]$ in data from $g$, and taking the sample average of the estimator in data from $g'$.
For example, suppose we have $N_g$ observations of $\{X_i, Y_i\}$ for populations $g \in {\{H, K\}}$, and let $\hat{M}_g(X)$ be an estimator for $\E_g  [ Y | X ]$.
Then}
{\color{red}
\begin{align}	
    \hat{\Delta}_Y
    \label{eq:NOBD_H_estimated}
    &=\underbrace{ \frac{1}{N_H} \sum_{i=1}^{N_H} Y_i - \frac{1}{N_K} \sum_{i=1}^{N_K} \hat{M}_H(X_i)}_{\hat{E}^{(H)}} 
     + \underbrace{\frac{1}{N_K} \sum_{i=1}^{N_K} \hat{M}_H(X_i) - \frac{1}{N_H} \sum_{i=1}^{N_H} Y_i}_{\hat{U}^{(H)} },
\end{align}
}
\textcolor{red}{implements \cref{eq:OB_H_Counterfactual}.}

\textcolor{red}{It is common to choose linear regression models for $\hat{M}_g(X)$, and refer to the decomposition as the OBD regardless of reference group choice.
We emphasize that the use of linear regression is a modeling choice, and that it is possible, and may even be desirable, to choose nonlinear estimators for $\E_g  [ Y | X ]$.
For instance, \citet{quintas2024multiply,flachaire2025decomposing,mao2025double} explore generalizations of the OBD that use machine learning estimators for $\E_g  [ Y | X ]$, and derive asymptotic properties for such procedures.
To distinguish the practice of using nonlinear estimators from typical choice of linear regression, we will refer to it as the nonlinear Oaxaca-Blinder decomposition (NOBD).}
