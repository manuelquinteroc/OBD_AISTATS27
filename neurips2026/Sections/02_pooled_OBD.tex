...
\label{sec:ob_real}
\textcolor{red}{Practitioners typically implement \cref{eq:OB_H_Counterfactual,eq:OB_K_Counterfactual} 
by computing an estimator $\hat{M}_g(X)$ for $\mathbb{E}_g[Y|X]$, and taking its sample average in data from $g'$.
For example,}
{\color{red}
\begin{align}	
    \hat{\Delta}_Y
...
\textcolor{red}{It is common practice to choose linear regression models for $\hat{M}_g(X)$, and refer to the decomposition as the OBD regardless of reference group choice.
We emphasize that the use of linear regression is a modeling choice, rather than a restriction on the data generating process.
It is possible, and may even be desirable, to choose nonlinear estimators for $\hat{M}_g(X)$.
To distinguish the choice of nonlinear estimator from typical practice, we will refer to it as the nonlinear Oaxaca-Blinder decomposition (NOBD).}
