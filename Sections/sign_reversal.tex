...
While our focus above is on the unexplained component, the explained component can also exhibit sign reversals across reference groups. Recall from \cref{eq:OB_H_decomp,eq:OB_K_decomp} that the explained component takes the form $\Delta\mu^T \beta_{g}$, where $\Delta\mu := \mu_H - \mu_K$ and $g \in \{H,K\}$ denotes the reference group. We formalize the corresponding notion of a sign flip and give the analogous characterization.
