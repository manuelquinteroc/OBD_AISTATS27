...
\textcolor{red}{As a first step toward describing the OBD, we illustrate the index number problem by framing it as a counterfactual exercise as in \citet{fortinLemieuxFirpo2011}.}
...
\textcolor{red}{Existing responses to the index number problem do not resolve it without relying on arbitrary choices or compromising the interpretation of results, and there seems to be no consensus response; see \cref{sec:existing_index_number} for a detailed discussion.
These responses include defining a new reference group by aggregating $H$ and $K$ \citep{oaxacaRansom1998,neumark1988}, and averaging the two components as a Shapley value would \citep{Shapley1953}; we show that the choice of reference group can reverse the sign of either component, so such averages combine two questionable quantities.}
%\textcolor{red}{In practice, we also do not see a consensus approach, even within fields of study; we survey how published work chooses a reference group in \cref{sec:existing_index_number}.}

%%% MOVED to \cref{sec:existing_index_number} (Appendix A.3): the survey below. The two responses above are
%%% already covered in more depth there, so they are dropped rather than duplicated. To reverse: uncomment.
%REV First, approaches like those of \citet{oaxacaRansom1998,neumark1988} define a new reference group by aggregating $H$ and $K$. However, there are many ways to perform this aggregation, introducing a new, still-arbitrary choice.
%REV Second, use of Shapley values \citep{Shapley1953} could call for averaging the explained or unexplained components for each reference group.
%REV However, we show that the choice of reference group can reverse the sign of either component, complicating their interpretation. 
%REV Shapley-style approaches thus average two questionable quantities and obscure information when the averaged components have opposite signs.
%REV In practice, we do not see a consensus approach, even within fields of study.
%REV For instance, in economic analyses of wage gaps, some argue that a single reference group makes the most sense to use in context, as in \citet{dinardoFortinLemieux1996}'s study of unionization, the minimum wage, and economic inequality.
%REV Often, the choice is a single reference group that aggregates the two original references in some way \citep{oaxacaRansom1994,reimers1983,cotton1988,kassenboehmer2014distributional,sharafRashad2016,rahimiNazari2021,Allen2022,bachan2022genderchancellor}.
%REV However, we also observe published work that takes either men or women as the reference group \citep{nielsen2000wage,piazzalunga2019increase,leythienne2021paygapeu,tao2024gender,paradaContzen2025}.
%REV Some papers report results for both references \citep{ONeill2006,fortinLemieuxFirpo2011,sharafRashad2016,rahimiNazari2021}. 
%REV Still others report results for just one reference, with no argument for excluding the other reference \citep{zhang_2019, ayubiShahbaziKhazaei2024,paradaContzen2025,singleton2016,lamJamiesonMittinty2021,leythienne2021paygapeu,alHanawiNjagi2022,tao2024gender}.
...
\begin{align}	
    ...
    &=\underbrace{ \frac{1}{N_H} \sum_{n=1}^{N_H} Y_n - \frac{1}{N_K} \sum_{n=1}^{N_K} \hat{M}_H(X_n)}_{\hat{E}^{(H)}} 
     + \underbrace{\frac{1}{N_K} \sum_{n=1}^{N_K} \hat{M}_H(X_n) - \textcolor{red}{\frac{1}{N_K} \sum_{n=1}^{N_K} Y_n}}_{\hat{U}^{(H)} }.
\end{align}
