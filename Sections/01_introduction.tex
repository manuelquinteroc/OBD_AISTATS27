...
The observed differences in mortality could be due to differences in these risk factors between the two groups (distribution of $X$) or to differences in how well the hospital manages these risk factors (distribution of $Y|X$).
...
Since the original paper \citep{oaxaca1973}, it has been well known that the OBD requires choosing one of the two groups (Patients Groups $K$ or $H$ in our example) as a reference, and that the choice of reference affects the numerical result of the OBD.
This dependence on the reference choice is sometimes known as the ``index-number problem.''
...
Some argue that a single reference group makes the most sense to use in context, as in \citet{dinardoFortinLemieux1996}'s study of unionization, the minimum wage, and economic inequality.
...
Others effectively construct a new reference group by aggregating the original two groups in various ways \citep{neumark1988,oaxacaRansom1994,reimers1983,cotton1988,sharafRashad2016,rahimiNazari2021,Allen2022}.
...
e.g., in our example, both references might attribute the mortality differences partly to the distribution of risk factors, differing only in magnitude.
In this case, all of the approaches above should yield similar qualitative conclusions.
...
e.g., suppose one reference attributes the mortality difference to the difference in risk factors and the other does not.
While the index-number problem states allows for different numerical answers, we are not aware of a systematic investigation
into whether the choice of reference groups can yield different \emph{substantive} conclusions. 
We fill this gap with a characterization of when the OBD's substantive conclusions are sensitive to the reference group, and a theoretical and empirical investigation of how common such sensitivity is.
...
space --- and provide intuition for when this non-uniformity might occur (\cref{sec:no_sign_flips}). We suggest best practices for data analysts in \cref{sec:conclusion}.
